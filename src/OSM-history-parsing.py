...
class TimelineHandler(osm.SimpleHandler):
    """ class TimeLineHandler: encapsulates the recovery of elements inside the OSM history. This history is composed of nodes, ways and relations, that have common attributes (id, version, visible, timestamp, userid, changesetid, nbtags, tagkeys). Nodes are characterized with their latitude and longitude; ways with the nodes that composed them; relations with a list of OSM elements that compose it, aka their members (a member can be a node, a way or another relation). We gather these informations into a single attributes named 'descr'. The timeline handler consider the OSM element type as well (node, way or relation). OSM history dumps do not seem to update properly 'visible' flag of OSM elements, so this handler recode it according to elements property.
    """
    def __init__(self):
    ...
    def node(self,n):
        """ Node recovery: each record in the history is saved as a row in the element dataframe. The features are the following: id, version, visible?, timestamp, userid, chgsetid, nbtags, tagkeys, elem type ("node") and geographical coordinates (lat, lon)"""
        nodeloc = n.location
    ...
    def way(self,w):
        """ Way recovery: each record in the history is saved as a row in the element dataframe. The features are the following: id, version, visible?, timestamp, userid, chgsetid, nbtags, tagkeys, elem type ("way") and a tuple (node quantity, list of nodes)"""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 OSM-history-parsing.py <osmfile.pbf>")
        sys.exit(-1)
    filepath = sys.argv[1]
    ...
    colnames = ['id', 'version', 'visible', 'ts', 'uid', 'chgset', 'ntags', 'tagkeys', 'elem', 'descr']

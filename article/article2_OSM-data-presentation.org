...
For the article remainder, we use the following acronyms:
...
+ simple because you just has to download the =osh= or =pbf= history
  dump file from [[https://planet.openstreetmap.org/][Planet OSM website]] (=osm= format refers to latest
  data, whereas =osh= refers to history data).
